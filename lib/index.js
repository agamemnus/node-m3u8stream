...
module.exports = (playlistURL, options) => {
  ...
  const requestQueue = new Queue((segment, callback) => {
    ...
    if ('byteRangeStart' in segment && segment.byteRangeStart !== undefined && (!(segment.byteRangeStart == 0 && segment.byteRangeEnd == 0))) {
     var rangeHeaders = {
      'Accept-Ranges': 'bytes',
  ...
  const refreshPlaylist = () => {
    ...
    parser.on('keyfile', (method, keyfilename) => {
     ...
     let prefix = ((keyfilename.substr(0, 8) == 'https://') || (keyfilename.substr(0, 7) == 'http://'))
                   ? ''
                   : (playlistURL.substr(playlistURL, playlistURL.lastIndexOf('/')) + '/')
     let keyuri = prefix + keyfilename
